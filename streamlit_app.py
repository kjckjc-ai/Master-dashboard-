...
# ---- PAGE SETUP ----
st.set_page_config(page_title="Combined School Dashboard", layout="wide")
...
# ---- NAVIGATION ----
page = st.sidebar.radio("Navigation", ["Trust Overview", "National View"])

# ---- TRUST OVERVIEW SECTION ----
if page == "Trust Overview":
    ...
    st.title("Trust Overview Dashboard")
...
elif page == "National View":
    st.title("National School Overview")

    st.subheader("Summary Statistics")
    col1, col2, col3 = st.columns(3)
    col1.metric("Total Schools", len(df))
    col2.metric("Total Pupils", int(df['NumberOfPupils'].dropna().astype(float).sum()))
    col3.metric("Average FSM %", f"{df['PercentageFSM'].dropna().astype(float).mean():.1f}%")

    st.subheader("Breakdown by Establishment Type")
    type_counts = df['TypeOfEstablishment (name)'].value_counts().reset_index()
    type_counts.columns = ['Establishment Type', 'Count']
    fig = px.bar(type_counts, x='Establishment Type', y='Count', title='School Type Distribution')
    st.plotly_chart(fig, use_container_width=True)

    st.subheader("Geographic Spread by LA")
    la_counts = df['LA (name)'].value_counts().reset_index()
    la_counts.columns = ['Local Authority', 'Count']
    fig = px.bar(la_counts.head(20), x='Local Authority', y='Count', title='Top 20 Local Authorities by Number of Schools')
    st.plotly_chart(fig, use_container_width=True)
